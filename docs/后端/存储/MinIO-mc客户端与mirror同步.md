...
`mc`（MinIO Client）是官方命令行工具，相当于对象存储界的 `aws cli` / `rsync`。本文聚焦安装、`alias` 配置以及最常用的 `mc mirror` 同步/备份能力，可以和《MinIO 专用备份服务器目录规范》配合落地。

## 一、mc 是什么

`mc` 提供了一套类似 Unix 命令的语法，常用的有：
...
`alias` 相当于给一个 MinIO 实例起个短名，之后所有命令不用重复写地址和密钥。
...
参数顺序：`alias 名称`、`endpoint`、`accessKey`、`secretKey`。
...
- `mc` 是 MinIO 官方命令行客户端，语法贴近 Unix 命令。
- 先 `mc alias set` 配置别名，后续命令省去地址与密钥。
