...
备份工具的选择很多，先看常见候选的定位对比：

| 工具 | 类型 | 为什么（不）选它 |
| --- | --- | --- |
| `mc mirror` | 对象级 S3→S3 镜像 | ✅ MinIO 官方客户端，按 ETag / 大小做增量，贴合 MinIO，无额外依赖 |
| `rclone` | 通用对象存储同步 | ✅ 功能强大、后端多；但非 MinIO 官方，S3 兼容细节需额外配置 |
| `restic` / `borg` | 块级去重备份（仓库式） | ❌ 备份成私有仓库格式，恢复依赖其工具，无法被「另一个 MinIO」直读 |
| `duplicati` | 桌面级增量备份 | ❌ 偏个人桌面场景，非服务端自动化首选 |
| `velero` | Kubernetes 卷备份 | ❌ 面向 K8s 集群资源，单机 Docker 场景杀鸡用牛刀 |
| Bucket Replication | 服务端原生复制 | ✅ 生产集群首选，但需多节点，单机不可用 |

> **核心选择逻辑**：`mc mirror` 是 MinIO 官方客户端，天然按对象 ETag / 大小做增量，且备份端仍是「可直接读写的 MinIO」而非私有格式仓库——既能灾备、又能随时直读或反向恢复，是单机 Docker 场景性价比最高的选择。

### 方案一：对象级同步 `mc mirror`（主备份，最推荐）
...
> 本章落地的是第二章**方案一**（`mc mirror` 对象级同步），采用**定时增量**（Cron 调度）而非 `--watch` 实时监听——两者取舍详见第七章。脚本主体只做「选型结论 + 工程化封装」，5.7 日志轮转、5.8 健康巡检属外围增强，与备份主逻辑解耦。
...
### 5.7 备份日志轮转（logrotate，防止日志无限膨胀）〔外围增强〕
...
综合前面的选型分析，对单机 Ubuntu + Docker 的个人 / 小团队项目，不必上 Bucket Replication，用 `mc mirror` 定时同步即可：
