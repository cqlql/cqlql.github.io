...
> | **HTTPS 条件** | ✅ **已确认** | Traefik 有 `websecure` 但只有默认自签证书 → 选择**不暴露**，只走 port-forward（§5.4.1） |
> | **故障演练** | ⏳ **未执行** | §6 阶段 4 的 9 项演练，是验证手段不是已完成项 |
> | **业务侧修正** | ⏳ **阻塞** | `passup` 命名空间尚未上集群，阶段 1 与多副本采集验收无法进行（§10.4） |
>
> **换句话说**：架构与集群内实施都已定稿并落地；剩下的是**业务侧接入**与**故障演练**。
...
> 与 1 处设计修正，见 §十。**下一步是业务侧（阶段 1）+ 故障演练（阶段 4）。**
>
...
✓ 14 条告警规则已加载
✓ 钉钉链路端到端打通（dingtalk-webhook 返回 {"status":"ok"}，群里收到测试消息）
```
...
| # | 项 | 阻塞原因 | 解阻条件 |
| :--- | :--- | :--- | :--- |
| 1 | **阶段 1 业务侧修正**（backend limit / ECS 日志契约 / 拓扑分布） | `passup` 尚未部署到集群 | 后端上集群 |
| 2 | 多副本采集验收、`\| level = "ERROR"` 日志验收 | 同上 | 同上 |
| 3 | **故障演练**（§6 阶段 4 的 9 项） | 依赖阶段 1 的数据 | 同上 |
| 4 | **集群外看门狗** | 脚本已就绪，但 Prometheus/Loki/Alertmanager 都是 ClusterIP、集群外访问不到；备份机 IP 也未知 | 先定暴露方式（LoadBalancer 或 Ingress）+ 拿到备份机地址 |
| 5 | 备份盘告警 | `BACKUP_NODE_EXPORTER_IP` 未填 | 填上备份机 IP |
| 6 | 双栈并行 / 下线宿主机旧栈 | 前置条件（阶段 4）未完成 | 阶段 4 通过 |
