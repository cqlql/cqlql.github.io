...
> | **故障演练** | 🔶 **部分完成** | 不依赖业务侧的 5 项（4/5/6/7/9）已真跑并留证据；1/2/3 被业务侧阻塞，8 被看门狗阻塞（§10.7） |
...
| 3 | **故障演练 1 / 2 / 3**（删 Pod / 滚动发布 / 业务节点宕机） | 依赖阶段 1 的数据 | 同上 |
| 4 | **集群外看门狗** | 脚本已就绪，但 Prometheus/Loki/Alertmanager 都是 ClusterIP、集群外访问不到；备份机 IP 也未知 | 先定暴露方式（LoadBalancer 或 Ingress）+ 拿到备份机地址 |
| 5 | **故障演练 8**（停污点节点） | 验证目标是「看门狗报 Prometheus 不可达」，而看门狗未部署（见 #4） | 同 #4 |
| 6 | 备份盘告警 | `BACKUP_NODE_EXPORTER_IP` 未填 | 填上备份机 IP |
| 7 | 双栈并行 / 下线宿主机旧栈 | 前置条件（阶段 4）未完成 | 阶段 4 通过 |
