...
| 组件 | requests | limits | PVC |
| :--- | :--- | :--- | :--- |
| Prometheus | 200m / 512Mi | 1 / 1Gi | 20Gi（保留 15d） |
| Loki | 200m / 512Mi | 1 / 1Gi | 20Gi |
| Grafana | 100m / 128Mi | 500m / 512Mi | 2Gi |
| Alertmanager | 50m / 64Mi | 200m / 256Mi | —（不持久化，告警无需留存） |
| Promtail（DaemonSet） | 50m / 128Mi | 200m / 256Mi | — |
| node-exporter（DaemonSet） | 50m / 64Mi | 200m / 128Mi | — |

- 存储类沿用 `local-path`，与业务同理：单节点无迁移需求，够用；
- Prometheus 保留期 `--storage.tsdb.retention.time=15d`，避免撑满磁盘；
- 建议给 `monitoring` 命名空间加 `ResourceQuota`，防止日志量突增时无限膨胀。
...
kubectl top nodes                         # 剩余资源是否够（约 1.5Gi 内存余量）
```
