...
core.StripingReduce = function () {
    ...
    this.start = function (to, fn) {
        ...
        function back() {
            ...
            if (Math.abs(vel) > 0.009) stopId = setTimeout(back, times);
        }
...
core.ImgEntirelyLoad = function () {
    /*
    属性事件，未发生前可更改设置清除。
    兼容性：包括ie6的所有
    参考代码：core.ImgEntirelyLoad
...
core.ImgEntirelyLoad.prototype.setName = function () {
    this.stop();
};


core.ImgEntirelyLoad.prototype = {
...
//升序
//将更改源数组(强调，此处文字没错)
core.systemSort = function (array) {
