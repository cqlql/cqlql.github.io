...
common.confirm_v1 = function (param) {
    ...
    var
        title = param.title === undefined ? '' : param.title,
        describe = param.describe === undefined ? '' : param.describe,
        btnsHtml = param.btnsHtml ? param.btnsHtml : '<a class="button mr10" href="javascript:;">确认</a><a class="gray_btn" href="javascript:;">取消</a>',
...
# method
    ...
    @param content [jq,elem,string] 要填充的内容正文 
    @param * onSucceed [function] 弹窗成功，可操作元素情况。传入内容的父元素jq对象
    @params * hasCloseBtn [bool] 是否有关闭按钮。false表示没有。默认有。
    @param * onCloseBtn [function] 重写关闭按钮事件
    @param * width 弹窗宽度，默认430

    @@ common.popup.close 关闭
    @example
        common.popup.close();
    @raise 固定删除content参数填充的内容
*/
common.popup = (function () {
    ...
    function Popup() {
        ...
        var jBox = $('<div class="popup"><div class="bg"></div><div class="win"></div></div>').appendTo(bJq).hide(),
            jWinBox = jBox.children('.win'),
        ...
        function center() {
            var left = (jBox.width() - wW) / 2,
                top = (jBox.height() - wH) / 3;
        ...
        this.show = function (params) {
            ...
            var
                ...
                hasCloseBtn = params.hasCloseBtn === undefined ? 1 : params.hasCloseBtn,
                onCloseBtn = params.onCloseBtn,

                onSucceed = params.onSucceed === undefined ? function () { } : params.onSucceed,

                width = params.width === undefined ? 430 : params.width,
            ...
            jConBox = $('<div class="con"><div class="top"><div class="top_m"><span class="tit">' + title + '</span><b>✕</b></div></div><div class="fn">' + content + '</div></div>');

            jWinBox.append(jConBox).width(width);
            ...
            // 关闭按钮
            jCloseBtn = jTop.children('.top_m').children('b');

            if (hasCloseBtn) {
                if (onCloseBtn) jCloseBtn.click(onCloseBtn);
                else jCloseBtn.click(close);
                jCloseBtn.mousedown(function () { return false; }).show();
            }
...
# method
    ...
    @example
        common.popup.close();
*/
common.confirm_v2 = function (params) {
    var title = params.title,
        description = params.description,
        onBtns = params.onBtns,
        hanCloseBtn = params.hanCloseBtn,
        onCloseBtn = params.onCloseBtn;

    common.popup({
        title: title,
        content: '<div class="confirm_box_v2"><div class="des"><i></i>' + description + '</div>\
                    <div class="btns">\
                        <button class="button">确认</button>\
                        <button class="button gray_btn">取消</button>\
                    </div></div>',
        onSucceed: function (jElem) {
            var jBtns = jElem.children().children('.btns').children();
            if (onBtns.length === 1) {
                jBtns.eq(0).on({
                    click: onBtns[0]
                });
...
common.msg = (function () {
    ...
    function resize() {
        ...
        function test() {
            ...
            var num = ~~((2/5*winH - unnecessary) / itemH);
...
common.pager = (function () {
    ...
    //清除 /page/2  location.href.replace(/\/page\/[\d]+/
    //清除 ?page=123 或 &page=123 location.href.replace(/[?&]page=[^&]*/
...
//等宽居中 显示
common.imgCenterByBoxEW = function (params) {
    var
    ...
    return { left: x, width: w, height: h };
};
    ...
    return function (param) {
        ...
        core.imageSizeExcu(src, function (d) {
            ...
            if (config || !boxJq) {
                ...
                switch (config) {
                    case 'imgCenterByBox':
                        imgJq.css(common.imgCenterByBox_v1({
                            boxWidth: boxW,
                            boxHeight: boxH,
                            width: d.w,
                            height: d.h,
...
common.imgFullShowSlide = function (params) {
    ...
    jBox.mouseenter(function () {
        ...
        if (domXY === undefined) domXY = jBox.offset();
...
jQuery.fn.extend({
    imgFullShowMove: function (isRoll) {
        isRoll = isRoll === undefined ? true : isRoll;
            ...
            */
            ...
            this.excu = function (params) {
                ...
                if (curIndex !== undefined) {
                    if (jCurAnimeBox) jCurAnimeBox.fadeOut(200);
...
@param jBox banner模块最外层元素
@param animeType [string] 动画类型
    fade 渐变
    move 移动
    down 上下
@param * listBtn [number] 列表按钮 选项。默认值为1
    0 没有按钮 
    1 有按钮。没数字
    2 有按钮。有数字
@param * arrow [number] 是否有箭头。默认 0
    0 没有
    1 有
@time * time [number] 自动播放间隔时间。默认不自动播

*/
common.banner_v3 = (function () {
    ...
    function unParam(param, def) {
        return param === undefined ? def : param;
    }

    function bannerTimer(jBox, fn, time) {
    ...
    var animateMove = (function () {
        ...
        return core.isIE6789 ? function (jMove, x, y) {
            jMove.animate({
                left: x,
                top: y
            }, {
            ...
            jMove.css({ transition: '.5s', transform: 'translate3d(' + x + 'px,' + y + 'px,0px)' });
        };
    })();
    ...
    var Anime = {
        fade: function (params) {
            ...
            this.excu = function (params) {
                ...
                jItems.eq(curIndex).fadeOut(500);
                jItems.eq(index).fadeIn(500);
            };

            this.ini = function () {
                jItems.eq(0).fadeIn(500);
            };
        },
        ...
        move: function (params) {
            var
                ...
                jItems = params.jItems,
                count = params.count,
                boxW = unParam(params.boxW, jBox.width()),
                curRealIndex;

            var cssX = core.isIE6789 ? function (x) {
                jMove.css({ left: x });
            } : function (x) {
                jMove.css({ transition: '0s', transform: 'translate3d(' + x + 'px,0px,0px)' });
            };
            ...
            this.excu = function (params) {
                ...
                setTimeout(function () { animateMove(jMove, -index * boxW, 0); }, 0);
                curRealIndex = index;
            };

            this.ini = function () {
                var jLastItem = jItems.eq(-1).clone(),
                    jFirstItem = jItems.eq(0).clone(),

                    jOtherItem = jLastItem.add(jFirstItem),
                    jImg;
                ...
                jMove.prepend(jLastItem).append(jFirstItem);

                jOtherItem.each(function () {
        ...
        down: function (params) {
            ...
            this.excu = function (params) {
                animateMove(jMove, 0, -params.index * boxH);
            };
            ...
            this.ini = function () {
                ...
                jMove.css({ position: 'relative' }).hide().fadeIn();
    ...
    function banner(params) {
        ...
        jListBox = jBox.children(),
        jItems = jListBox.children(),

        listBtn = unParam(params.listBtn, 1),
        arrow = unParam(params.arrow, 0),

        //banner 高宽
        boxW = params.boxW,
        boxH = params.boxH,

        animeType = unParam(params.animeType, 'fade'),
        ...
        //记录 图片是否被进行加载处理。处理过将不会再处理
        imgLoaded = {},

        anime,

        animeStatus = 0,//0 停止  1 进行中
        ...
        function oneIni() {
            ...
            jBtns && jBtns.eq(index).addClass('active');
        ...
        function loadImg(index) {
            if (!imgLoaded[index]) {
                ...
                jItems.eq(index).find('img').each(function () {
                    this.src = $(this).attr('data-src');
                });
            }
        }
        ...
        function btnIni() {
            ...
            if (listBtn) {
                var html = '<div class="b_btn">', i,
                    delayExcu = new DelayExcu();
                ...
                if (listBtn === 1) {
                    ...
                    for (i = 0; i < count; i++) {
                        html += '<a href="javascript:;" data-index="' + i + '">' + (i + 1) + '</a>';
                    }
                }

                html += '</div>';

                jBtns = $(html).appendTo(jBox).children().click(function () {
                    btnSelect($(this).attr('data-index'));
                }).hover(function () {
                    var jThis = $(this);
                    delayExcu.excu(function () {
                        btnSelect(jThis.attr('data-index'));
                    });
                }, function () {
                    delayExcu.clear();
                });
        ...
        function arrowIni() {
            ...
            var html, jArrows,
                validLoad;
            if (arrow) {
                validLoad = new ValidExcu();

                html = '<a href="javascript:;" class="b_arrows l"></a><a href="javascript:;" class="b_arrows"></a>';

                jArrows = $(html).appendTo(jBox).click(function () {
                    var eThis = this;

                    arrowChange(jArrows.index(eThis));
        ...
        function arrowChange(add) {
            ...
            animeStatus = 1;
            setTimeout(function () { animeStatus = 0; }, 550);
        ...
        if (time) {
            bannerTimer(jBox, function () { arrowChange(1); }, time);
        }
    }

    return function (params) {

        params.jBox && params.jBox.each(function () {
            params.jBox = $(this);
            banner(params);
        });
...
*/
common.dragWin = function (params) {
    ...
    var
        ...
        stripingReduceX = new core.StripingReduce(),
        stripingReduceY = new core.StripingReduce();
...
common.coupletAd = function (minHeight) {
    ...
    minHeight = minHeight === undefined ? 192 : minHeight;
    ...
    jQuery.fn.extend({
        ...
        promptIn: function () {
            ...
            return this.each(function () {
                ...
                $hintTxt = $('<span style="padding:0px;">' + placeholder + '</span>');
...
common.sideBox = function () {
    ...
    function posHandle() {
        ...
        function update() {
            var
                ft = jFooter.length ? jFooter.offset().top : 0,
                bt = jBox.offset().top,
...
common.navFreed = function () {
    ...
    function up(to) {
        if (curTo === undefined) return;

        var first = to[0],
            second = to[1] === undefined ? 0 : to[1],
...
common.scrollBar = function (params) {
    ...
    function conDrag() {
        ...
        stripingReduce = new core.StripingReduce();
    ...
    function moveByCon(o) {
        ...
        if (barO === curBarO && o === curConO) return;
...
common.search = function () {
    var jBox = $('#headerSearchBoxGraver');

    function goto(val, type) {
        var url,
            val;
    ...
    if (jBox.length) {
        ...
        var jGo = $('.btn', jBox),
            jIn = $('.inp', jBox),
			jSelectBox = $('.select_customization',jBox),
            jShowBtn = jSelectBox.children().eq(0),
            jShowBox = jSelectBox.children().eq(1),
            //jSelectItems = $('li', jSelectBox),
            jTabs = $('li', jBox),
            jCurTab = jTabs.eq(0),
            tabs = (function () {
                var obj = {};
		...
		jSelectBox.hover(function(){
            //jShowBox.css("display","block");
            jShowBox.show();
        },function(){
            //jShowBox.css("display","none");
            jShowBox.hide();
        });

        jTabs.eq(curIndex).addClass('active');
        ...
        function goExcu() {
            goto($.trim(jIn.val()), jCurTab.attr('data-type'));
        }
        ...
        jTabs.click(function () {
            change($(this));
			jShowBox.hide();
            changeSelect(this.d_index);
            jShowBtn.html(this.innerHTML);
			goExcu();
        });
...
common.comment = function (jCommentBox) {
    ...
    (function () {
        var id = curPagams.id_reference;

        if (id === 'null') {
            id = location.href.match(/\/id\/([\d]+)/);

            if (id === null) id = location.href.match(/id=([\d]+)/);

            id = id[1];
        }

        curPagams.id_reference = id;
    ...
    function eTopHandle() {
        ...
        this.ini = function (params) {
            ...
            jTop.html('<ul class="h_l">\
                <li class="h_hello"><i></i> <em>欢迎您,' + nickname + '</em></li>\
...
# 初始
    ...
    @example
        downMenuBlurClose = new common.DownMenuBlurClose({
            jHover: jSelectBox,
            jShowBtn: jShowBtn,
            jShowBox: jShowBox,
            onShow: function () { },
...
*/
common.DownMenuBlurClose = function (params) {
    var
...
//#region jq Ajax 错误
common.jQAjaxErrorMsg = function (XMLHttpRequest, textStatus, errorThrown) {
    switch (textStatus) {
...
var loadImgCenter=new common.LoadImgCenter(10,176,5);
loadImgCenter.excu(0,function(index){});
*/
common.LoadImgCenter = function (count, itemV, showCount) {
    ...
    function excu(v, callback) {
        var

            startIndex,
            tIndex;

        if (loadedNum >= count) return false;

        startIndex = Math.floor(Math.abs(v / itemV));

        for (var i = 0; i < showCount; i++) {

            tIndex = startIndex + i;

            if (tIndex >= count) break;

            if (loadData[tIndex] === undefined) {
    ...
    this.excu = function (v, callback, ready) {
...
/*
地址参数清理
    能够清理类似这种 /list/page/1
    或这种 /list?page=1
@example
    urlParamsClear('page,keyword','/list/page/1/keyword/123/index?page=1&keyword=123');
@param multiple [string] 要清理的参数名字，多个用逗号隔开
@param * url [string] url地址，默认当前地址(即location.href)
@return [string] 处理后的字符串
*/
common.urlParamsClear = function (multiple, url) {
    url = url === undefined ? location.href : url;

    $.each(multiple.split(','), function (i, n) {

        var reg = new RegExp(n + '/*[^/?#]*/*'),
            reg2 = new RegExp(n + '=[^&]*&{0,1}');
...
//////项目的一些
common.getThumb = function(src, w) {
    var i = src.lastIndexOf('.');
    return src.substring(0, i) + '_' + w + src.substr(i);
};
...
$(function () {
    ...
    */
    var loginOutHandle = {
        index: function () {
            var jOtherBtn = $('#frontShowIndexQuit');

            if (jOtherBtn.length) return;

            var jTopBtn = $('#frontShowQuit'),
                jShow = jTopBtn.children('a'),
                jOShow = jOtherBtn.children('a');

            common.loginOut({
                jBtn: jTopBtn.add(jOtherBtn),
                onStart: function () {
                    jShow.css({
                        width: 46,
                        'text-align': 'left',
                ...
                loadingCallback: function (len, str) {
                    jShow.add(jOShow).html('退出中' + str);
                }
            });
        },
        ...
        general: function () {
            ...
            if (jTopBtn.length === 0) return;
            var jShow = jTopBtn.children('a');
    ...
    function navFreedHandle() {

        var w = wJq.width(), h = wJq.height(), st = wJq.scrollTop(),
            navFreed = new common.navFreed(),
            delayExcu = new core.delayExcu(),
                data = [];

        function excu() {
            var st = wJq.scrollTop() + h / 4 + 40, noExcu = true;

            for (var i = 0, len = data.length; i < len; i++) {
                if (st > data[i][0] && st < data[i][1]) {
                    delayExcu.clear();
                    navFreed.go(data[i][2], 1);
                    noExcu = false;
                    break;
                }
            }
            ...
            if (noExcu) {
                delayExcu.excu(function () { navFreed.close(); }, 200);
        ...
        $('.nav_freed_fn').each(function () {
            var jElem = $(this),
            elemTop = jElem.offset().top,
            elemMaxTop = elemTop + jElem.height(),
            navVal = jElem.attr('data-nav').split(',');

            data.push([elemTop, elemMaxTop, navVal]);
        });
    }

    //图片处理
    function imgOptimizeShow() {

        $('.imgFullShowMove').imgFullShowMove();

        $('.imgFullShow').imgFullShow();

        $('.imgDelayLoad').each(function () {
            ...
            common.rollExcu({
                ...
                callBack: function () {
                    ...
                    jBox.removeClass('imgDelayLoad');

                    jImg.hide().fadeIn()[0].src = jImg.attr('data-src');
    ...
    //前台展示页面 头部显示隐藏
    function frontShowHeader() {
        var jFixedHeader = $('#frontShowHeaderFixed'),
            curTop = 0,
            isShow = true,
            goTop=new GoTo();
        ...
        function GoTo() {
            ...
            this.goExcu = function (start, getRealTimeVal, fn) {
                ...
                if (going === false) {
                    ...
                    setTimeout(function () {
                        ...
                        var s = (start - getRealTimeVal()) / ((new Date()).getTime() - startTime) * 1000;
        ...
        function eventFn() {
            ...
            if (st === 0) {
                jFixedHeader.stop().css({ top: 0 });
                isShow = true;
            }
            else if (st > curTop) {
                if (isShow) {
                    jFixedHeader.stop().animate({ top: -40 });
                    isShow = false;
                }
            }
            else {
                if (isShow === false) {
                    goTop.goExcu(curTop, function () {
                        return wJq.scrollTop();
                    }, function () {
                        jFixedHeader.stop().animate({ top: 0 });
                        isShow = true;
                    });
                }
            }
    ...
    (function () {
        ...
        //部分头部登录绑定
        if ($('#ajaxFrontHeaderLogin').length) {
            common.popLoginBindBtn({
                jPopBtn: $('#ajaxFrontHeaderLogin')
            });
        }

        common.sideBox = new common.sideBox();
        ...
        //登出
        loginOutHandle.member();
        loginOutHandle.specialIndex();
        loginOutHandle.general();
        loginOutHandle.index();

        //统计
        setTimeout(function () { core.addScript('http://tajs.qq.com/stats?sId=31542930'); }, 100);

        if (core.isIE67) {
            bJq.append('<div class="top_alert warning"><i></i><span>你正在使用 Internet Explorer 的过期版本。 本网站不再支持 Internet Explorer 6 和 Internet Explorer 7。 <a class="a_type1" href="http://windows.microsoft.com/zh-cn/windows/upgrade-your-browser">更新</a> 或 <a class="a_type1" href="http://www.google.com.hk/chrome/">安裝 Google 网页浏览器</a>。 </span></div>');
        }
    })();
